...
function check_or_push() {
    ...
    if [[ ${CHECK} == "--check" ]]; then
        echo "Checking $TAG ..."
        version=`docker run mochat/mochat:$TAG php -v`
        echo $version | grep -Eo "PHP \d+\.\d+\.\d+"
        swoole=`docker run mochat/mochat:$TAG php --ri swoole` && echo $swoole | grep -Eo "Version => \d+\.\d+\.\d+" || echo "No Swoole."
    fi

    if [[ ${CHECK} != "--check" ]]; then
        echo "Publishing "$TAG" ..."
        # Push origin image
        docker push mochat/mochat:${TAG}
    fi
...
if [[ ${TASK} == "publish" ]]; then
    # Push base image
    TAGS="7.4-alpine-v3.9-base 7.4-alpine-v3.10-base 7.4-alpine-v3.11-base 7.4-alpine-v3.12-base"
    for TAG in ${TAGS}; do
        check_or_push $TAG
    done
fi
